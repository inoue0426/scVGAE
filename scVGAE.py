...
def run_model(input_data, verbose=False, device=False):
    ...
    optimizer_name = 'Adam'
    optimizer = getattr(torch.optim, optimizer_name)(model.parameters(), lr=params['lr'], )
    ...
    for epoch in epochs:
        ...
        loss = compute_loss(x, x_recon, z_mean, z_dispersion, z_dropout, params['alpha']).to(device)
        optimizer.zero_grad()
        loss.backward()
        optimizer.step() 
